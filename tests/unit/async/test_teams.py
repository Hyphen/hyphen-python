...
class TestTeamsAsync:
    ...
    async def test_revoke_team_role(self, owner_client, leader_client, normal_client):
        ...
        lead_marketing = [x for x in await leader_client.teams.list() if x.name == "marketing"][0]
        normal_member, *_ = await lead_marketing.members.assign_role("teamLead", [normal_member])
        ...
        lead_member = [x for x in await owner_marketing.members.list() if x.id == lead_member.id][0]
